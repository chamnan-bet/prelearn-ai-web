<template>
  <div class="flex flex-col min-h-full">
    ...
    <header class="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between sticky top-0 z-40 shrink-0">
      <div>
        <h1 class="font-semibold text-slate-900 text-lg leading-tight">My Progress</h1>
        <p class="text-xs text-blue-400 mt-0.5">Bac II readiness score</p>
      </div>
      <div class="flex items-center gap-2">
        <button class="w-10 h-10 rounded-lg bg-blue-100 border-blue-200 flex items-center justify-center text-blue-500 hover:bg-blue-300 transition" aria-label="Profile">
    ...
    <main class="p-6 flex-grow">
      ...
      <div class="flex flex-col md:flex-row gap-4 mb-7">
        ...
        <div class="bg-orange-400 rounded-2xl px-6 py-7 text-white flex flex-col items-center justify-center text-center md:w-48 shrink-0">
          ...
          <p class="text-3xl font-semibold leading-none mb-1">{{ streakDays }} {{ streakDays === 1 ? 'day' : 'days' }}</p>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 flex-grow">
          <div class="bg-white border border-slate-200 rounded-2xl p-5 flex flex-col">
            <div class="text-3xl font-semibold text-slate-900 mb-1">{{ masteredCount }}</div>
            <p class="text-slate-400 text-xs font-medium mb-3 flex-grow">Patterns mastered</p>
            <p class="text-emerald-500 text-xs font-medium ">+{{ masteredThisWeek }} this week</p>
          </div>
          <div class="bg-white border border-slate-200 rounded-2xl p-5 flex flex-col">
            <div class="text-3xl font-semibold text-emerald-500 mb-1">{{ predictedScore }}%</div>
            <p class="text-slate-400 text-xs font-medium mb-3 flex-grow">Predicted score</p>
            <p class="text-emerald-500 text-xs font-medium ">+{{ predictedScoreDiff }}% this week</p>
          </div>
          <div class="bg-white border border-slate-200 rounded-2xl p-5 flex flex-col">
            <div class="text-3xl font-semibold text-slate-900 mb-1">{{ patternsRemaining }}</div>
            <p class="text-slate-400 text-xs font-medium mb-3 flex-grow">Patterns remaining</p>
            <p class="text-orange-500 text-xs font-medium ">Focus on Math</p>
          </div>
          <div class="bg-white border border-slate-200 rounded-2xl p-5 flex flex-col">
            <div class="text-3xl font-semibold text-slate-900 mb-1">+{{ marksSaved }}</div>
            <p class="text-slate-400 text-xs font-medium mb-3 flex-grow">Marks saved so far</p>
            <p class="text-red-400 text-xs font-medium ">vs. before PreLearn</p>
          </div>
        </div>
      </div>

      <!-- Weak areas -->
      <p class="text-[11px] font-medium text-slate-400 uppercase tracking-widest mb-3">
      ...
      <div class="bg-white rounded-2xl border border-slate-200 shadow-sm mb-6">
        ...
        >
          ...
          <span class="text-xs font-semibold text-red-600 whitespace-nowrap">{{ area.marks }}</span>
        </div>
      </div>
      ...
      <div class="w-full">
        <RouterLink
          ...
          class="w-full flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 active:scale-[0.98] text-white font-medium  px-7 py-3.5 rounded-xl transition"
        >
          Study weak areas
